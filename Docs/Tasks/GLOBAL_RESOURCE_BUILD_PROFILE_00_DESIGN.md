...
The Resource Manager save path is still authoring metadata only. It does not build AssetBundles, publish remote manifests or mutate generated `StreamingAssets` outputs. Player artifacts are generated by the Unity menu after validation.
...
The first write endpoint is limited to saving the global build profile after validation. Bundle Plan remains preview-only; generated Player files are still written by the Unity Editor build menu, not by the web tool.
