...
> 资源管理上游设计：`Docs/Tasks/CHARACTER_RESOURCE_LIBRARY_00_DESIGN.md`
> 边界：本任务只负责角色包 provider 的 Unity 导入状态和 Unity 可实例化资产映射；它不是全局 Authoring Resource Manager，也不负责 Runtime Resource Plan。运行时预加载、AudioCue 和角色资源生命周期由 Authoring Compiler 输出的 Runtime Resource Catalog / Character Resource Plan / Runtime Resource Orchestrator 负责。
