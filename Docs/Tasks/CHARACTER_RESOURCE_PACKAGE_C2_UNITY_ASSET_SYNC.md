...
> 边界：本任务只负责 Unity 导入状态和 Unity 可实例化资产映射；运行时预加载、AudioCue 和角色资源生命周期由资源库设计中的 Runtime Resource Catalog / Character Resource Plan / Runtime Resource Orchestrator 负责。
