...
`SocketEntry` 的语义是稳定挂点引用，不是世界空间位置。`LocatorId` 应解析到骨骼、locator、body part 或 primitive anchor；运行时 / importer 根据当前骨骼姿态和 authoring local pose 合成最终 transform。武器挂载点、VFX 挂点和 camera 挂点都不应默认绑定到 model root 原点。
...
`CharacterBodyPartConfig` 的 locator 语义：

- `PartKind=Bone` 时，`LocatorId` 必须能解析到代表该部位的骨骼或骨骼 locator，例如 `bone.Head`、`bone.RightHand`。
- `PartKind=Primitive` 时，`LocatorId` 可以是 primitive anchor，例如 `primitive.sphere.center`；用于 Slime、召唤物、简单碰撞代理等无骨骼 Actor。
- `PartKind=Virtual` 时，`LocatorId` 可以为空或指向组合 locator，但必须由 resolver / validator 明确是否可用于 collider 或 socket parent。
- Body part 是 collider、hit zone、socket 默认父空间的稳定层。碰撞框、挂载点和 trace 需要跟随动画时，应绑定到 body part 代表骨骼 / locator 的局部空间，而不是保存 model root 原点偏移。
- 如果 body part 更换代表骨骼，引用该 `PartId` 的 colliders / sockets 应保持引用不变，只重新解析到新的骨骼空间；删除 body part 才会使这些引用失效。
...
- `PartKind=Bone` 的 body part 必须能解析到代表骨骼 / locator。
- collider 的 `PartId` 必须存在；其局部 pose 应解释为相对 body part 代表骨骼 / locator，而不是 model root 原点。
- socket 的 `LocatorId` / authoring `bonePath` 必须能解析；武器 socket 的 identity pose 应是默认握持姿态。
