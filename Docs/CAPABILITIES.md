...
| UI Toolkit Runtime Showcase | ✅ M6 accepted | `RuntimeVerticalSliceConfigWindow` 编辑配置资产；HUD 支持手动控制、Patch / Mod Package 加载、Ability 重建、Old/New 配置对比、Diagnostic View、Mini Game Feedback，以及 `MxStatBar` / `MxCommandButton` / `MxStatusBadge` / `MxEventLog` / `MxPanelTabs` 复用控件 token。按 #529 共存策略，Showcase / Debug / Editor / Validation surface 默认继续使用 UI Toolkit |
| FairyGUI Runtime UI Adapter | ✅ Runtime Slice | `MxFramework.UI` 提供 noEngine view contract / navigator / command contract；`MxFramework.UI.FairyGUI` 通过 `MxFramework.Resources` 加载 package bytes 并打开 `MxRuntimeHud` / `MxStoryDialog`；`MxFramework.UI.FairyGUI.Manifest` 校验 schema、package bytes、ResourceCatalog、component/control/command/localization drift。FairyGUI adapter 是可选 player-facing path；UI Toolkit diagnostics/showcase 不被自动替换 |
...
→ UI：`Assets/UI/MxFramework/Showcase/GameplayShowcase.uxml`, `Assets/UI/MxFramework/Tetris/TetrisPlayableDemo.uxml`, `Assets/UI/MxFramework/Breakout/BreakoutPlayableDemo.uxml`, `FGUIProject/assets/MxRuntimeHud`
