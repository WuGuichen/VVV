...
| S2 | 公共 API、跨模块依赖、Runtime / Gameplay / Config / Combat 主流程 | Spec Issue、GitNexus 或替代影响面分析、接口文档同步、PR 审查 | 不可省略 Human Gate |
...
8. **影响面分析**：按 `Docs/GITNEXUS.md` 使用 GitNexus；如果索引过期，先 `Tools/GitNexus/gitnexus.sh analyze`。
...
- 默认扫描整个仓库。
- 读取或修改 `Library/`、`Temp/`、`Logs/`、`.gitnexus/`、`.codex/cache/`。
...
`Docs/WORKFLOW.md` 和 `Docs/GITNEXUS.md` 是权威入口，但普通开发任务不要求全文读取；只有流程、权限、GitNexus、提交、PR、Harness 或 Agent 行为不清楚时才读取相关章节。
...
| 改动范围 | 最低验证 |
| --- | --- |
| 纯文档 / 流程 | `git diff --check`，必要时 GitNexus 或手动影响面分析 |
| 纯 C# Core / Runtime | 相关 EditMode / dotnet 测试，优先 GitNexus `detect-changes` |
| 公共 API / 跨模块依赖 | 优先 GitNexus 影响面分析，相关模块测试，接口文档同步 |
...
GitNexus 优先使用；如果索引过期、工具失败或当前环境不可用，必须说明原因，并用 `git diff`、`rg`、源码阅读和相关测试补足影响面分析。
