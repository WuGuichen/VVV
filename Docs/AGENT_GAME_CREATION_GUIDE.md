...
8. 如果任务会新建或修改场景、材质、Shader、Volume、相机或表现资源，读取 `Docs/RENDERING_PIPELINE.md`
...
- 场景和材质在当前 URP 项目基线下渲染正常，不依赖 Built-in Render Pipeline 专属 Shader、旧 Post Processing Stack 或未记录的自定义管线资产。
