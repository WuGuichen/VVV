...
| `PressureBand` / `PressureHysteresis` | Component-native pressure 状态段和默认迟滞阈值值对象 |
| `GameplayPosturePressureComponent` | Component-native 姿态压力状态，包含最大压力、当前压力、恢复参数、当前状态段和 broken 标记 |
| `GameplayPosturePressureRequest` | 姿态压力系统的 transient 输入请求，使用 `GameplayEntityId` 和 delta，不携带 Combat/UI/AI 类型 |
| `PressureBandChangedEvent` / `PostureBreakEvent` | 姿态压力状态段变化和破韧 typed event payload |
| `GameplayPosturePressureSystem` | Resolution phase system，消费姿态压力请求、执行迟滞状态段计算和恢复，并发布 typed events |
| `GameplayGuardPressureComponent` | Component-native 防御压力状态，复用 pressure band / hysteresis，包含恢复参数和 broken 标记 |
| `GameplayGuardPressureRequest` | 防御压力系统 transient 输入，表达 delta、是否仍在 blocking、source 和 trace |
| `GuardBreakEvent` | 防御压力进入 Broken 段时的 typed event payload |
| `GameplayGuardPressureSystem` | Resolution phase system，消费防御压力请求、执行恢复、发布 GuardBreak，并可写入 GuardBroken status |
| `GameplayArmorIntegrityComponent` | Component-native 护甲完整度状态，包含最大/当前完整度和 broken 标记，不自动恢复 |
| `ArmorBreakEvent` | 护甲完整度归零时的 typed event payload |
...
| `CharacterGameplaySpawnDefinitionBuilder` | 将 imported character runtime binding 转成 Gameplay component spawn definition 的 Character runtime adapter |
| `CharacterGameplayRuntimeBootstrap` / `CharacterRuntimeEntityRegistry` | Character runtime 的 noEngine 组合根和 stable character id 到 live `GameplayEntityId` registry |
| `CharacterGameplayAbilityRegistryBuilder` | Character ability registry bootstrap 入口；当前 slice 显式记录 deferred ability gap |
...
Guard / Armor pressure components:

- `GameplayGuardPressureComponent` 与 `GameplayPosturePressureComponent` 使用相同的 `PressureBand` 和默认 hysteresis 阈值；`GameplayGuardPressureSystem` 只消费显式 `GameplayGuardPressureRequest`，不直接读取 Combat 类型。
- `GameplayGuardPressureRequest.IsBlocking == true` 会刷新 `LastPressureFrame`，即使本次 delta 为 0；因此角色持续格挡时不会触发恢复，停止格挡并超过 `RecoveryDelayFrames` 后才恢复。
...
Character runtime bootstrap foundation v0：

- `CharacterGameplaySpawnDefinitionBuilder` 位于 `MxFramework.CharacterRuntimeSpawn`，把 `CharacterRuntimeBinding` / `CharacterResolvedProfile` / imported config 中的 character id、loadout id、team、lifecycle 和 attribute profile 转成 `GameplayComponentSpawnDefinition`。
- Builder 输出的 initializer 只写入 `GameplayIdentityComponent`、`GameplayTeamComponent`、`GameplayLifecycleComponent` 和 `GameplayAttributeSetComponent`；不读取 Unity object，不注册 Combat body，不绑定 view。
- Character team string 通过 deterministic stable hash 转为 `GameplayTeamComponent.TeamId`；空 team 保持 neutral `0`。
- Character attribute float values 在进入 component runtime 时转成 int `GameplayAttributeValue`，以 component runtime 的 int attribute contract 为准。
- `CharacterGameplayAbilityRegistryBuilder` 当前只返回空 `GameplayComponentAbilityRegistry` 和 deferred diagnostics，记录 resolved effective ability ids；能力定义映射留给后续 ability recipe slice，不在 #408 内伪造 effect。
- `CharacterGameplayRuntimeBootstrap` 是 noEngine runtime owner，持有 `RuntimeHost`、`RuntimeCommandBuffer`、`GameplayRuntimeModule`、`GameplayComponentWorld`、spawn registry、ability registry 和 `CharacterRuntimeEntityRegistry`。
- Live character spawn 必须 enqueue `SpawnComponentEntity` command，再由 `GameplayRuntimeModule` drain command 并从 `ComponentEntityCreated` event 取得真实 generation-safe `GameplayEntityId`。
- `CharacterRuntimeEntityRegistry` 只保存 `stableCharacterId <-> live GameplayEntityId <-> Combat ids <-> CharacterControlEntityRef` 映射；它不是 Gameplay state source of truth，也不替代 `GameplayComponentWorld` lifecycle。
- Destroy 使用 `DestroyComponentEntity` command path 清理 component world，再移除 registry handle；不直接从 MonoBehaviour callback 修改 component stores。
...
Component posture pressure v0：

- `GameplayPosturePressureComponent` 是纯 Gameplay component，不引用 Unity、Combat、UI 或 Runtime AI Planner；Combat 命中事实必须由后续 bridge 转换成 `GameplayPosturePressureRequest`。
- `MaxPressure > 0`，`CurrentPressure` clamp 到 `0..MaxPressure`；`RecoveryRate` 和 `RecoveryDelayFrames` 不能为负数。
- 默认状态段阈值按 `MaxPressure` 百分比计算：Pressed 25%，Cracked 50%，Critical 75%，Broken 100%。向下恢复使用默认迟滞出口，避免临界值附近闪烁。
- `GameplayPosturePressureSystem` 运行在 `GameplaySystemPhase.Resolution`，默认 priority 为 `70`，早于 buff cleanup 和 lifecycle cleanup。
- `Enqueue(in GameplayPosturePressureRequest)` 是 transient input；system tick 会先消费请求，再处理符合 delay 的自然恢复，最后清空本帧请求队列。
- `Delta > 0` 会更新 `LastPressureFrame`；`Delta < 0` 表示外部主动释放压力，不更新最后受压帧。自然恢复只在 `frame - LastPressureFrame > RecoveryDelayFrames` 后按 `RecoveryRate` 每 tick 生效。
