...
PORT="${1:-${CHARACTER_STUDIO_PORT:-4873}}"
PACKAGE_RELATIVE="${2:-${CHARACTER_STUDIO_PACKAGE:-$DEFAULT_PACKAGE}}"
OPEN_BROWSER="${CHARACTER_STUDIO_OPEN_BROWSER:-1}"
URL="http://127.0.0.1:${PORT}/Tools/MxFramework.CharacterStudio/web/"
STATE_URL="http://127.0.0.1:${PORT}/api/character/state"
...
wait_and_open_url() {
  ...
  warn "CharacterStudio server did not become ready in 30 seconds. Open manually after it starts: $URL"
}
...
if [[ -n "${MXFRAMEWORK_FBX2GLTF:-}" && ! -x "$MXFRAMEWORK_FBX2GLTF" ]]; then
  warn "MXFRAMEWORK_FBX2GLTF is set but not executable: $MXFRAMEWORK_FBX2GLTF"
fi

if [[ ! -d "$SCRIPT_DIR/node_modules/three" || ! -d "$SCRIPT_DIR/node_modules/fbx2gltf" ]]; then
  warn "CharacterStudio npm dependencies are missing. GLB preview may fall back, and FBX conversion will be unavailable."
  warn "Run once: npm --prefix Tools/MxFramework.CharacterStudio install"
fi

if is_port_in_use; then
  if command -v curl >/dev/null 2>&1 && curl -fsS "$STATE_URL" >/dev/null 2>&1; then
    printf 'CharacterStudio-compatible Authoring server is already running on port %s.\n' "$PORT"
    printf 'URL: %s\n' "$URL"
...
printf 'MxFramework CharacterStudio\n'
printf 'Root   : %s\n' "$ROOT_DIR"
printf 'Package: %s\n' "$PACKAGE_RELATIVE"
printf 'Port   : %s\n' "$PORT"
printf 'URL    : %s\n' "$URL"
