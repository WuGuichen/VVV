...
The `动画配置` panel is a temporary animation authoring bridge. It creates package-local `applicationConfig.animationGroups[]` entries from visible animation resources, lets authors add/remove groups, edit clips, and configure 2D BlendSpace points, then connects `applicationConfig.animationProfiles[].slots[]` to those groups through `animationGroupId`. Source animation assets are still selected through the same `ResourceFieldSpec` picker used by model fields. Package/runtime animation choices also remain in `applicationConfig.resourceKeys` so they participate in `AnimationWarmup`; Unity editor-only animation assets are saved as `ResourceSelectionRef` until a later import/compile step resolves them into runtime resources. This panel must migrate to the standalone Animation Editor described in `Docs/Tasks/ANIMATION_EDITOR_00_DESIGN.md`; after migration CharacterStudio should only select, preview, and link to existing animation profiles/groups.
