...
The `动画配置` panel is now a CharacterStudio handoff surface, not an animation source editor. It shows the character's current animation profile slots, lets authors choose animation resources through the same `ResourceFieldSpec` picker used by model fields, and links to the standalone Animation Editor in `Tools/MxFramework.AnimationEditor/web/`. `applicationConfig.animationGroups[]` is treated as legacy/read-only summary data here; Group, Clip, Blend, and Timeline Event source content belongs to the Animation Editor and its `config/animation_authoring.json` package document. Package/runtime animation choices still remain as `ResourceSelectionRef` data so the compiler can place them into `AnimationWarmup`.
